...
export async function createDemographics(data, userId) {  
  ...
  if (existing) throw new ValidationError("Demographics already exist for this academic year");  
...
export async function updateDemographics(id, data, userId) {  
  ...
  if (!existing) throw new NotFoundError("Demographics not found");  
  if (existing.school.adminId !== userId) throw new ForbiddenError("Not authorized to update these demographics");  
...
export async function deleteDemographics(id, userId) {  
  ...
  if (!existing) throw new NotFoundError("Demographics not found");  
  if (existing.school.adminId !== userId) throw new ForbiddenError("Not authorized to delete these demographics");  
