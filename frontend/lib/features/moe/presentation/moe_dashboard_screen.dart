...
class MoeDashboardScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MoeDashboardScreen> createState() => _MoeDashboardScreenState();
}
...
class _MoeDashboardScreenState extends ConsumerState<MoeDashboardScreen> {
  ...
  Future<void> _downloadCsv() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'CSV download is currently available only on the web version.',
        ),
      ),
    );
  }
                            ...
                            children: [
                              ...
                              Wrap(
                                ...
                                children: [
                                  ...
                                  FilledButton.icon(
                                    onPressed:
                                        _downloadCsv, // Changed from _copyCsv
                                    icon: const Icon(
                                        Icons.file_download), // Updated icon
                                    label: const Text(
                                        'Download CSV'), // Updated label
                                  ),
                                ],
                              ),
                            ],
...
class _UsersByRoleChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (data.isEmpty) {
      return const Card(
          child: Padding(padding: EdgeInsets.all(16), child: Text('No data')));
    }
    ...
    if (data.isEmpty) {
      return const Card(
          child: Padding(
              ...
              child: Text('No subcity data available')));
    }
                            ...
                            children: [
                              ...
                              FractionallySizedBox(
                                widthFactor: (percentage > 0
                                        ? percentage
                                        : 0.05)
                                    .toDouble(), // Minimum 5% for visibility
                                child: Container(
                                  height: 40,
                                  decoration: BoxDecoration(
                                    color: theme.colorScheme.primary,
...
class _TopSchoolsByRatingChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (schools.isEmpty) {
      return const Card(
          child: Padding(
              padding: EdgeInsets.all(16), child: Text('No schools yet.')));
    }
...
class _MostFollowedLeaderboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (schools.isEmpty) {
      return const Card(
          child: Padding(
              padding: EdgeInsets.all(16), child: Text('No followers yet.')));
    }

    final maxFollowers =
        schools.map((s) => s.followers).reduce((a, b) => a > b ? a : b);
        ...
        child: Column(
          ...
          children: [
            ...
            ...schools.asMap().entries.map((entry) {
              ...
              final percentage =
                  maxFollowers > 0 ? school.followers / maxFollowers : 0;
                    ...
                    Row(
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () =>
                              context.go('/schools/${school.schoolId}'),
                        ),
                      ],
                    ),
                  ],
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('MoE School Ranking (Top 10)',
